...
with tf.device('/gpu:0'):  
    ...
    class AlphaGAN(object):
        ...
        def build_directory(self):
            ...
            subfolders = [f[0] for f in os.walk(f'{gan_name}/{self.dataset}/alpha-d{self.alpha_d}-g{self.alpha_g}-{IMG_SIZE}')]
            folders = [f for f in subfolders if f.startswith(f'{gan_name}/{self.dataset}/alpha-d{self.alpha_d}-g{self.alpha_g}-{IMG_SIZE}/v')]
    ...
    opts = Option(gan_type='alpha', alpha=3.0, seed=42, c_type='discrete', n_epochs=300, dataset='mri', loss_type='alpha', lambda_d=1.0, lambda_c=0.1, num_images= 128, gp=True, gen_lr=0.0002, dis_lr=0.0002, q_lr=0.0002, gp_coef=5.0, alpha_d=args.alpha_d, alpha_g=args.alpha_g, k=2.0, shifted=False, l1=False)
    ...
    gan.train(f'/data/user1/AlphaGANMRI/cleaned128by128/Training/{args.type}', f'/data/user1/AlphaGANMRI/cleaned128by128/Training/{args.type}')
    gan.save_generated_images(1)
